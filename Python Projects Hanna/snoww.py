...
class SnowFlake():
    ...
    def __init__(self, size, position, wind=True):
        self.size = size
    ...
    def fall(self, speed):
        ...
        y += speed
        if self.wind == True:
            x = self.position[0] - 1#  - the x direction of the snowflake changes
        self.position = (x, y)
...
# Snow List
snow_list = [SnowFlake(2, (random.randrange(0, 700), 10), True), SnowFlake(2, (random.randrange(0, 700), 10), True)]
# -------- Main Program Loop -----------
while not done:
    # --- Main event loop
    ...
    for snow in snow_list:
        snow.draw()
        snow.fall(1)
    snowow = SnowFlake(2, (random.randrange(0,700), 10), True)
    snow_list.append(snowow)
    # End Snow
